...
from PyQt5.QtWidgets import (QMainWindow , QAction, QWidget, QLineEdit, QMessageBox, QAbstractItemView, QApplication, 
                                                           QTableWidget, QTableWidgetItem, QGridLayout, QFileDialog, QMenu, qApp, QInputDialog, QPushButton)
...
#        self.tableView.setSelectionBehavior (QAbstractItemView.SelectRows )
    ...
    def createMenuBar(self):
        ...
        self.toolbar = self.addToolBar('Add hash')
        exitAction = QAction(QIcon('addhash.png'), 'Add hash', self)
                     
        #exitAction.setShortcut('Ctrl+Q')
        #exitAction.triggered.connect(qApp.quit)
        exitAction.triggered.connect(self.addhash)
        
        #self.toolbar.addAction((QIcon.fromTheme("document-save-as"), "Add hash ...",  self.addhash))
        #self.addAction(self,(QIcon.fromTheme("document-save-as"), "Add hash ...",  self.addhash))
    ...
    def addhash(self):
        ...
        item.setText('hash')  
        self.tableView.setHorizontalHeaderItem(count, item)         
        #Раб self.tableView.model().setHeaderData(count, Qt.Horizontal, 'hash')
        
        for row in range(self.tableView.rowCount()):
            rowdata = []
            for column in range(self.tableView.columnCount()-1):
                item = self.tableView.item(row, column)
                if item is not None:
                    rowdata.append(item.text())
                else:
                    rowdata.append('')
            newItem = QTableWidgetItem(self.get_hesh(" ".join(rowdata)))
            self.tableView.setItem(row,count, newItem)
            print(" ".join(rowdata))
            
        
        #print(dir(self.tableView.model))
        #print(self.tableView.verticalHeaderItem(count))
        #print(dir(self.tableView.verticalHeaderItem))
        #rows = set(cell.row() for cell in self.tableView.selectedIndexes()) # set to remove duplicates, otherwise use a list
        #labels = [self.tableView.verticalHeaderItem(r).text() for r in rows]
        #print(labels)
        #https://stackoverflow.com/questions/60735357/how-to-get-selected-header-label-from-qtablewidget-item

    def get_hesh(self,stroka):
        bo = bytes(stroka, 'utf-8')
        hash_object = sha256(bo)
        """
